...
const createRequest = async (requestInfo) => {
  ...
  return new Promise( async(resolve, reject) => {
    ...
    try {
      ...
      const requesterUsername_res = await User.find({username: newRequest.requesterUsername});
      const requesteeUsername_res = await User.find({username: newRequest.requesteeUsername});

      if( ride_res == null ){ throw "ride with rideID not found"; }
      if( requesterUsername_res.length == 0) { throw "specified requesterUsername not found"; }
      if( requesteeUsername_res.length == 0 ) { throw "specified requesteeUsername not found"; }
    } catch(err) {
...
const updateRequestStatus = async (requestID, authUsername, status) => {
  ...
  return new Promise( async(resolve, reject) => {
    ...
    try{
      ...
      if (!request_res) 
        { errFlag = 404; throw "Specified request not found"; }
      else if (request_res.archived) 
        {  errFlag = 400; throw "Ride has already been archived"; } 
      else if (request_res.status !== "pending") 
        { errFlag = 400; throw "Ride has already been " + request_res.status; } 
      else {
        // switch
        switch(status){
          case "approved": {
            if( authUsername != request_res.requesteeUsername ) {  errFlag = 401; throw "Unauthorized request action: You are not the requestee"}
          ...
          case "cancelled": {
            if( authUsername != request_res.requesterUsername ) {  errFlag = 401; throw "Unauthorized request action: You are not the requester"}
            request_upd = await Request.findOneAndUpdate(filter, update, options);
            await Noti.create({
          ...
          default: {
            errFlag = 400; throw "invalid status to update";
          }
        }
        return resolve(request_upd);
      }
    } catch (err) {
      if(errFlag) { return reject(Error(errFlag, err)); }
      return reject(Error(500));
    }
  });
};
...
const decrementRemindCount = (requestID, authUsername) => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if(request_res == null)
        { errFlag=404; throw null; }
      if(request_res.requesteeUsername != authUsername) 
        { errFlag=401; throw "Unauthorized request action: You are not the requestee"; } 
      else if (request_res.reminders < 1)
        { errFlag=400; throw "Reminder count is already less than 1"; }
