...
describe("Testing leave a review notification", () => {
  ...
  test("Testing whether an expiry task is created for every review that can be sent out", async () => {
    ...
    expect(
      schedule.scheduledJobs[
        `expireAbilityToLeaveReviewTask.${ride._id}.driverUsername.passenger1`
      ]
    ).toBeTruthy();
    expect(
      schedule.scheduledJobs[
        `expireAbilityToLeaveReviewTask.${ride._id}.driverUsername.passenger2`
      ]
    ).toBeTruthy();
  });
  ...
  test("Testing that a review should be made public if one exists before the time for leaving a review expires", async () => {
    ...
    await scheduledTasks.expireAbilityToLeaveReviewTask(
      ride._id,
    ...
    expect((await User.findById(passenger1._id)).rating.sumOfAllRatings).toBe(
      14
    );
